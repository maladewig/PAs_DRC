...
source(here("code","3functionSheet.R"))

load(file=here(dataOut,"bpts.diff.cov.Rdata"))
bpts.diff.v <- vect(here(dataInt,"bpts15000DiffTMF.shp"))

s2PAs <- vect(here(dataPrep,"s2PAsClean.shp"))
s2PAs <- project(s2PAs,crs(bpts.diff.v))

# Load grd results from Orion and bpts 
load(here(dataOut,"r1aDef500_40000.Rdata"))
load(here(dataOut,"r1bDegr500_40000.Rdata"))
load(here(dataOut,"r1cCov500_40000.Rdata"))

PAs.df <- as.data.frame(vect((here(dataPrep,"s2PAsClean.shp"))))
...
save(r1AnnualATTs.df,file=here(dataOut,"r1AnnualATTs.df.Post2000.TMF.Rdata"))
...
bpoints <- bpts.diff$bpoint
  ...
  save(res.by.iucncat,file = here(dataOut,"res.by.iucncat.Rdata"))
p1 <- res.by.iucncat %>% 
...
save(bpts.diff,file=here(dataOut,"bpts.diff.cov.Rdata"))
...
save(res.PAs,file=here(dataOut,"PA.hetero.Rdata"))
load(file=here(dataOut,"PA.hetero.Rdata"))
...
# TABLE 1: MINING / LOGGING x CONSERVATION ------------------------------------
  ...
  ### Expl. permit ####
  mining_dbf <- vect(here(dataInt,"Heterogeneities","minconcessions_dbf.shp"))
  ...
  logg_conc <- vect(here(dataPrep,"Forest_concession_agreements.shp"))
...
#i2aDblFrontMin <- i2aDblFrontMin %>% filter(!is.na(year.mine0),year.mine0>2000)
...
DblMin <- DiffInDisc(depvar = "fcover",
  ...
  save(DblMin,file = here(dataOut,"DblMin20000.Rdata"))
  load(file = here(dataOut,"DblMin20000.Rdata"))
...
PAlines <- as.lines(s2PAs)
PAlines <- project(PAlines,bpts.diff.v)
s3bTimber <- vect(here(dataPrep,"Forest_concession_agreements.shp"))
...
save(res.timb.dbf,file = here(dataOut,"res.timb.dbf.Rdata"))
...
RDplots <- plots
saveRDS(plots, file=here(dataOut,"RDplots.RData"))
...
save(robust.df,file=here(dataOut,"PA.est.Rdata"))
load(file=here(dataOut,"PA.est.Rdata"))
...
save(DblMin,file = here(dataOut,"DblMinRob.Rdata"))
load(file = here(dataOut,"DblMinRob.Rdata"))
